		...
		<tbody>
			<s:iterator value="dataPage.data" status="st">
				<tr <s:if test="!#st.odd">class="trodd"</s:if>>
					...
					<td align="center">
					    <c:choose> 
					       <c:when test="${fn:contains(button, 'punch_apply_edit') && type ==1}">
					         <a href="<%=request.getContextPath()%>/core/punchApply/ComU_edit.do?id=<s:property value="punchApplyId"/>">修改</a>
						   </c:when>
						   <c:when test="${fn:contains(button, 'punch_edit') && type ==2}">
					         <a href="<%=request.getContextPath()%>/core/punchApply/ComU_edit.do?id=<s:property value="punchApplyId"/>">审核</a>
						   </c:when>
						 </c:choose>
						<c:if test="${fn:contains(button, 'punch_apply_del') && type ==1}">  
						<a href="javascript:doDel('<s:property value="punchApplyId"/>','');">删除</a>
						</c:if>
						 <c:choose> 
					       <c:when test="${fn:contains(button, 'punch_apply_load') && type ==1}">
						    <a href="<%=request.getContextPath()%>/core/punchApply/ComR_load.do?id=<s:property value="punchApplyId"/>">查看</a>
					       </c:when>
