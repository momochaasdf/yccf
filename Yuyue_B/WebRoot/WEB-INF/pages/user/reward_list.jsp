		...
		<tbody>
			<s:iterator value="dataPage.data" status="st">
				<tr <s:if test="!#st.odd">class="trodd"</s:if>>
					...
					<td align="center"><a
						href="<%=request.getContextPath()%>/core/reward/ComU_edit.do?id=<s:property value="rewardId"/>">修改</a>
						<a href="javascript:doDel('<s:property value="rewardId"/>','');">删除</a>
						<a
						href="<%=request.getContextPath()%>/core/reward/ComR_load.do?id=<s:property value="rewardId"/>">查看</a>
					</td>
				</tr>
			</s:iterator>
		</tbody>
