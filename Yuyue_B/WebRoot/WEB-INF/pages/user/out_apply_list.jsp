...
<script type="text/javascript">
	function doDel(id, name) {
		$.messager.confirm('删除确认', '你确认删除吗?', function(bt) {
			$("#id").val(id);
			if (bt) {
				doAction('outApplyForm', 'ComD_del', '')
			}
			;
		});
	}
		...
		<div class="o-mt">
			<h2 style="margin-top: 0;">
				<a href="<%=path%>/core/outApply/ComM_list.do"
					style="color: #cc0000">外出管理</a>
			</h2>
		</div>
	</div>
	<form action="#" method="post" id="outApplyForm" style="clear: both;">
			...
			<table cellpadding="0" cellspacing="0" class="navSearch"
				...
				<tr>
					...
					<td style="width: 30%"><input type="text"
						name="ioutApply.userName" value="${ioutApply.userName}" /></td>
					<th style="width: 20%">部门名称</th>
					<td style="width: 30%"><select type="text"
						name="ioutApply.departmentId">
		...
		<div class="navButton">
			...
			<input class="btAdd" value="新增"
				onclick="doAction('outApplyForm','ComC_add','')"
				style="color:#FFF;border-style:none;width:49px;height:25px;padding:0;background: url(<%=path%>/common/images/blue_bg.png)  no-repeat scroll 0px 0px transparent;text-align: center" />
		</div>
		...
		<tbody>
			<s:iterator value="dataPage.data" status="st">
				<tr <s:if test="!#st.odd">class="trodd"</s:if>>
					...
					<td align="center"><a
						href="<%=request.getContextPath()%>/core/outApply/ComU_edit.do?id=<s:property value="outApplyId"/>">修改</a>
						<a href="javascript:doDel('<s:property value="outApplyId"/>','');">删除</a>
						<a
						href="<%=request.getContextPath()%>/core/outApply/ComR_load.do?id=<s:property value="outApplyId"/>">查看</a>
					</td>
				</tr>
			</s:iterator>
		</tbody>
