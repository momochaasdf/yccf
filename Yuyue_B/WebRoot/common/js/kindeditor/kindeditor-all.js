...
function _mediaImg(blankPath, attrs) {
	var mediaHtml = '<iframe src="' + BaseData.path + '/js/cuplayer/media.jsp?CuPlayerFile=' + attrs.src + 
			'&SWFWidth=' + attrs.width +
			'&SWFHeight=' + attrs.height +
			'&CuPlayerWidth=' + attrs.width +
			'&CuPlayerHeight=' + attrs.height +
			( attrs.autostart === 'true' ? '&CuPlayerAutoPlay=yes' : '' )+
			'" class="media_iframe"  width="' + ( 30 + parseInt(attrs.width)) + '"' +
			'height="' + ( 30 + parseInt(attrs.height) ) + '"' +
			'id="mediaHtmlID"' +
			'style="overflow: hidden;border:0;" frameborder="0" scrolling="no" ></iframe>';
	return mediaHtml;
}
				...
				var uploadbutton = K.uploadbutton({
					...
					afterUpload : function(data) {
						...
						if (data.error === 0) {
							...
							$.colorbox.alert(self.lang('uploadSuccess'));
						} else {
							if(data.message === "fail")
								$.colorbox.alert("上传失败！");
							else if(data.message === "typeError")
								$.colorbox.alert("只能上传文件类型为:" + data.types);
							else
								$.colorbox.alert("操作失败！");
						}
					},
	...
	init : function(options) {
		...
		var settings = {
			...
			upload_success_handler : function(file, serverData) {
				...
				file.url = data.url;
				K('.ke-img', itemDiv).attr('src', IMG_PATH + "/" + file.url).attr('data-status', file.filestatus).data('data', data);
				K('.ke-status > div', itemDiv).hide();
			}
		};
