...
function _formatHtml(html, htmlTags, urlType, wellFormatted, indentChar) {
	...
	var tagStack = [];
	html = html.replace(re, function($0, $1, $2, $3, $4, $5, $6) {//这里循环匹配得到的标签，用正则表达式进行处理
		var full = $0,//这个是标签的整个内容
			startNewline = $1 || '',
			startSlash = $2 || '',//是否是结束标签“一般是/”
			tagName = $3.toLowerCase(),//标签的名称
			attr = $4 || '',//标签里面的内容
			endSlash = $5 ? ' ' + $5 : '',
			endNewline = $6 || '';
...
//这个方法是用于粘贴Word文档去除Word文档的格式
//(1)“/ig”(全文查找、忽略大小写)
//(2)"[\s\S]"任意字符（包括回车换行）出现任意多次
...
_extend(KCmd, {
	...
	insertimage : function(url, title, width, height, border, align) {
		...
		html += 'width="100%" style="float:left;"/>';
