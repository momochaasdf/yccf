...
$.formValidator = 
{
	...
	oneIsValid : function (id)
	{
		...
		///如果之前手动设置过该对象已经验证通过或不通过，则取上次结果,对应$.formValidator.setValid方法
		if(e.dom.validValueOld === e.dom.value && e.dom.isValid !== undefined){
			e.isValid = e.dom.isValid;
			if(e.dom.isValid)
				e.setting = e.settings[0];
			else {
				var aindex = e.dom.validatorAjaxIndex;
				e.setting = e.settings[aindex];
				e.errormsg = e.dom.errormsg;
			}
			return e;
		}
	...
	pageIsValid : function (validatorGroup)
	{
		...
		if(isValid && initConfig.debug){$.colorbox.alert("现在正处于调试模式(debug:true)，不能提交");}
	...
	ajaxForm : function(validatorGroup,option,formid)
	{
		...
		if(formid==undefined){
			...
			if(formid==""){$.colorbox.alert('表单ID未传入');return false;};
		};
		...
		setting.url =  ls_url;
		setting.data = ls_data;
		//设置回调函数
		var beforeSendTmp = setting.beforeSend;
		setting.beforeSend = function(jqXHR, configs){//再服务器没有返回数据之前，先回调提交按钮
			if(setting.buttons && setting.buttons.length > 0){setting.buttons.attr({"disabled":true})};
			return beforeSendTmp(jqXHR, configs);
		};
		var completeTmp = setting.beforeSend;
		setting.complete = function(jqXHR, textStatus){
			if(setting.buttons && setting.buttons.length > 0){setting.buttons.attr({"disabled":false})};
			completeTmp(jqXHR, textStatus);
		};
        $.ajax(setting);
	},
	...
	ajaxValid : function(e)
	{
		...
		var ls_url = setting.url;
		var ls_data = $.extend(true, {}, setting.data || {}, setting.dataFunc() || {});
		//获取要传递的参数
		...
		{	
			...
			beforeSend : function(jqXHR, configs){
				...
				}else if((typeof lb_ret)=="boolean" && !lb_ret){
					isValid = false;
					setting.isValid = true;
					$.formValidator.setTipState(elem,"onCorrect",settings[e.ajax].onCorrect);
				}else {
					isValid = false;
	...
	setValid : function(id, isValid, msg){
		///注意：此方法未完成 -- 手动让指定的元素验证通过或不通过
		var dom = $("#"+id).get(0);
		isValid = isValid ? true : false;
		dom.isValid = isValid;
		dom.validValueOld = dom.value;
		dom.errormsg = msg;
	    $.formValidator.setTipState(dom, isValid ? "onCorrect" : "onError", msg);
	},
	setAjaxValid : function(id, isValid, msg){
		///手动设置指定的ajax元素验证
		///isValid == true 则验证通过 否则验证失败
		var dom = $("#"+id).get(0)
			,aindex = dom.validatorAjaxIndex;
		dom.settings[aindex].isValid = isValid;
	    $.formValidator.setTipState(dom, isValid ? "onCorrect" : "onError", msg);
	}
		...
		if(srcTag == "input" || srcTag=="textarea")
		{
		    ...
		    jqobj.bind("keyup",function(){
		        if(this.validatorPasswordIndex > 0)
		        {
					...
					{
						$.colorbox.alert("密码强度校验失败,错误原因:变量passwordStrengthStatusHtml语法错误或者为设置)");
					}
	            }
		    });
...
var initConfig_setting = 
{
	...
	onAlert: function() {$.colorbox.alert(arguments[0])},
...
var ajaxForm_setting = 
{
	...
	cache : false,
	buttons : null,
	beforeSend : function(){return true;},		//return false;	不发ajax请求
