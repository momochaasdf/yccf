...
/**
 * This simple command line utility decodes files, directories of files, or URIs
 * which are passed as arguments. By default it uses the normal decoding
 * algorithms, but you can pass --try_harder to request that hint. The raw text
 * of each barcode is printed, and when running against directories, summary
...
public final class CommandLineRunner {
	...
	public static void main(String[] args) throws Exception {
		...
		for (String arg : args) {
			if ("--try_harder".equals(arg)) {
				...
				config.setPossibleFormats(COMMA.split(arg
						.substring(POSSIBLE_FORMATS_ARG.length())));
		...
		int numThreads = Math.min(inputs.size(), Runtime.getRuntime()
				.availableProcessors());
		int successful = 0;
		if (numThreads > 1) {
		...
		if (total > 1) {
			System.out.println("\nDecoded " + successful + " files out of "
					+ total + " successfully (" + (successful * 100 / total)
					+ "%)\n");
		}
	}

	// Build all the inputs up front into a single flat list, so the threads can
	// atomically pull
	// paths/URLs off the queue.
	private static void addArgumentToInputs(String argument, Config config,
			Queue<String> inputs) throws IOException {
		...
		if (inputFile.exists()) {
			if (inputFile.isDirectory()) {
				for (File singleFile : inputFile.listFiles()) {
					...
					// Recur on nested directories if requested, otherwise skip
					// them.
					...
					// Skip text files and the results of dumping the black
					// point.
					if (filename.endsWith(".txt")
							|| filename.contains(".mono.png")) {
						continue;
					}
	...
	// Manually turn on all formats, even those not yet considered production
	// quality.
	...
	private static void printUsage() {
		...
		System.err.println();
		System.err
				.println("usage: CommandLineRunner { file | dir | url } [ options ]");
		System.err
				.println("  --try_harder: Use the TRY_HARDER hint, default is normal (mobile) mode");
		System.err
				.println("  --pure_barcode: Input image is a pure monochrome barcode image, not a photo");
		System.err
				.println("  --products_only: Only decode the UPC and EAN families of barcodes");
		System.err
				.println("  --dump_results: Write the decoded contents to input.txt");
		System.err
				.println("  --dump_black_point: Compare black point algorithms as input.mono.png");
		System.err.println("  --multi: Scans image for multiple barcodes");
		System.err
				.println("  --brief: Only output one line per file, omitting the contents");
		System.err.println("  --recursive: Descend into subdirectories");
		System.err
				.println("  --crop=left,top,width,height: Only examine cropped region of input image(s)");
		StringBuilder builder = new StringBuilder(
				"  "
						+ POSSIBLE_FORMATS_ARG
						+ "barcodeFormat[,barcodeFormat2...] where barcodeFormat is any of: ");
