...
public final class BufferedImageLuminanceSource extends LuminanceSource {
			...
			if (EXPLICIT_LUMINANCE_CONVERSION) {
				...
				for (int y = top; y < top + height; y++) {
					...
					for (int x = 0; x < width; x++) {
						...
						buffer[x] = (306 * ((pixel >> 16) & 0xFF) + 601
								* ((pixel >> 8) & 0xFF) + 117 * (pixel & 0xFF) + 0x200) >> 10;
					}
				...
				if (image.getAlphaRaster() != null) {
					...
					for (int y = top; y < top + height; y++) {
						...
						if (rowChanged) {
							image.setRGB(left, y, width, 1, buffer, 0,
									sourceWidth);
						}
					}
				}
	...
	public LuminanceSource rotateCounterClockwise45() {
		...
		BufferedImage rotatedImage = new BufferedImage(sourceDimension,
				sourceDimension, BufferedImage.TYPE_BYTE_GRAY);
		...
		int newRight = Math
				.min(sourceDimension - 1, oldCenterX + halfDimension);
		int newBottom = Math.min(sourceDimension - 1, oldCenterY
				+ halfDimension);
